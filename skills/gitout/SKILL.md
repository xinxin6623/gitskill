...
intent:
  ...
  license_sensitivity: strict | tolerant | ignore  # 默认 strict，见下
  agent_friendly: required | preferred | ignore    # 默认 preferred，见下
  queries: []   # need 模式才在 Step 3b 生成；brand 模式由探针决定
```

**`license_sensitivity` 判定规则**（默认 `strict`，宁严勿松）：

| 用户输入特征 | license_sensitivity | 行为 |
|---|---|---|
| 出现"商用 / 公司 / 客户 / 团队 / 闭源 / 二次开发 / SaaS / 对外提供 / 部署给别人用" | `strict` | 自动 reject GPL/AGPL/SSPL；MIT/Apache/BSD/MPL 优先 |
| 明确说"个人玩 / 自己用 / 不分发 / 内网" | `tolerant` | GPL 项目降级提示但不淘汰，文档头部加协议风险段 |
| 明确说"无所谓 / 看 star / 不在乎协议" | `ignore` | 不做协议过滤 |
| **没暴露协议信号** | `strict`（默认） | 同 strict 行为 |

**`agent_friendly` 判定规则**（默认 `preferred`）：

| 用户输入特征 | agent_friendly | 行为 |
|---|---|---|
| 出现"AI agent / 自动化 / 程序调用 / API / CI/CD / Claude Code 维护" | `required` | 候选必须有 REST API 或文档化 CLI；纯 SSH / 纯 Web UI 项目降级 |
| 出现"自己手动用 / 浏览器点点 / 命令行手敲" | `ignore` | 不做加权 |
| **未暴露** | `preferred`（默认） | API/CLI 项目在 Step 5 排序加权 |

→ 这两个字段直接驱动 Step 2 的"是否主动追问"和 Step 5 的"诉求覆盖广度"权重，**不要遗漏**。
...
#### Step 2.1：协议风险主动追问（硬性触发）

在 Step 3 之前，**扫一遍初步候选名单**（用 description + license 字段即可，不用抓 README）：

- 命中条件：候选里**有任一 GPL / AGPL / SSPL 项目** 且 `intent.license_sensitivity != ignore`
- 动作：**强制 AskUserQuestion**，例如：
  > 候选里有 `<repo-name>`（GPL-3.0）。如果你打算对外提供服务（SaaS / 给客户部署 / 团队商用），GPL 可能有传染风险。要排除 GPL 类项目吗？
- 选项：
  1. 排除 GPL/AGPL/SSPL（推荐）
  2. 保留但标注风险
  3. 不在乎协议，全部保留

用户回答后**更新 `intent.license_sensitivity`** 并继续。

> **由来**：2026-05-26 git-self-host followup 反馈——首轮没主动问协议偏好，导致 Forgejo（GPL-3.0）出现在"相关周边"提及。详见 `followups/2026-05-26-git-self-host/feedback.md` 建议 1 + 3。
...
| **诉求覆盖广度** | **高** | 同时 cover 用户多个子诉求的项目优先（详见下） |
| 实现质量信号 | 中 | 代码可读性、README 实操内容 |
| 软偏好命中 | 中 | soft_preferences 命中数 |
| **协议契合度** | 中（strict 时升为高） | 按 `intent.license_sensitivity` 加减权 |
| **agent 友好度** | 中（required 时升为高） | 有 REST API / 文档化 CLI 加分 |
| 活跃度 | 低 | last_commit、contributor 数 |
| stars | 最低 | **明确降权** |

**"诉求覆盖广度"判定**：

- 把用户的需求拆成 N 个子诉求（如 git-self-host 案例 = {Git 平台, 依赖镜像, 网站部署}）
- 候选项 cover 的子诉求数越多排序越靠前
- **同一场景下，"一个项目顶仨"通常优于"三个项目各管一摊"**，避免推荐组合让用户装一堆软件
- 例外：用户明确说"喜欢小工具组合"或"反对万能型"时不加这条权重

**输出 3-5 个**（候选 ≥5 强制 5；<5 按实际）。

> **由来**：2026-05-26 git-self-host followup 反馈——首轮把 piku（只 cover 网站部署）排到 #1，把 Gitea（同时 cover Git 平台 + 依赖镜像 + 网站部署 via Actions）排到 #2，违背"一个项目顶仨"的常识。详见 `followups/2026-05-26-git-self-host/feedback.md` 建议 4。
...
## 📜 协议风险（**强制段，所有 entry 必须有**）
- **License：** <SPDX 标识，如 MIT / Apache-2.0 / GPL-3.0>
- **商用 / 魔改 / 闭源：** ✅ 无风险 | ⚠️ 有限制（说明）| ❌ 不允许
- **对外提供 SaaS / 给客户部署：** ✅ 无传染义务 | ⚠️ 可能触发开源义务（说明）

> GPL/AGPL/SSPL 类项目，此段必须用 `<span text-color="orange">⚠️</span>` 标注，并在头部加：
> `> ⚠️ **协议警告：本项目 GPL-3.0，如对外提供服务可能触发开源义务**`
...
5. **"为什么这次推它给你"三句必须齐**
6. **"📜 协议风险"段必须齐**——License 字段已经在 frontmatter `signals.license`，正文必须显式呈现一次；GPL/AGPL/SSPL 必须显式警告

> **由来**：2026-05-26 git-self-host followup 反馈——entry 里 license 字段只在 frontmatter，正文不强调，导致 Forgejo（GPL-3.0）的风险没被显眼标出。详见 `followups/2026-05-26-git-self-host/feedback.md` 建议 5。
...
- what: <...>
- 关键约束: <...>
- license_sensitivity: <strict|tolerant|ignore>
...
### 2026-05-26 · v2.1（followup-driven 升级）

来源：`followups/2026-05-26-git-self-host/feedback.md` 5 项建议全部落地。

| 改动 | 位置 | 解决什么 |
|---|---|---|
| intent 加 `license_sensitivity` 字段 | Step 1 | 协议风险维度漏抽取（Forgejo GPL 没被显眼淘汰） |
| intent 加 `agent_friendly` 字段 | Step 1 | AI 时代 agent 维护友好度漏抽取（soft-serve 没 API 没被标红） |
| 候选有 GPL 时强制 AskUserQuestion | Step 2.1 新增 | 不让用户裸装 GPL 项目就上 SaaS |
| Step 5 权重表加"诉求覆盖广度" + 协议契合度 + agent 友好度 | Step 5 | piku 不该排 Gitea 前面（一个能 cover 三诉求 vs 只 cover 一诉求） |
| entry 模板加"📜 协议风险"强制段 | Step 6 | License 字段从 frontmatter 升到正文显式呈现，GPL/AGPL 必须警告 |
| 结构化汇报加 license_sensitivity / agent_friendly 字段 | Step 7.4 | 让用户能在汇报里直接看到这两个维度的判定 |
